
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";

import { faStar } from "@fortawesome/free-solid-svg-icons";

const RentCards = () => {
  return (
    <div className=" mt-6">
      <h2 className="text-3xl mb-2 font-bold text-center mt-5">
        Bikes <span className="text-red-500">For</span> Rent
      </h2>
      <div className="flex justify-center mt-2 mb-6">
        <div className="h-2 w-12 rounded-lg text-center bg-red-500"></div>
      </div>
      <div className="grid grid-cols-5 gap-5">
        <div className="col-span-1"></div>
        <div className="col-span-3 grid grid-cols-3 gap-4 mt-6 mb-4">
          <div className="grid-cols-1 border border-red-500 p-5 border-r-2">
            <div>
              <img src="https://res.cloudinary.com/sayed-ltd/image/upload/w_1000,ar_16:9,c_fill,g_auto,e_sharpen/v1724601933/logan-weaver-lgnwvr-0fiuoIIfl9g-unsplash_1_moiaxv.jpg" alt="" />
            </div>
            <div className="flex">
                <div>
                    <h1>KTM Duke 1290</h1>
                    <p>location</p>
                    <button className="btn bg-red-500 text-white py-2 px-4 mt-3 font-bold">Book Now</button>
                </div>
                <div className="ml-auto text-right">
                    <h3>$100</h3>
                    <p className="text-sm">DAY</p>
                    <p className="mt-8">991 Reviews <FontAwesomeIcon className="text-yellow-500" icon={faStar} /> 5.0</p>
                </div>
            </div>
          </div>
          <div className="grid-cols-1 border border-red-500 p-5 border-r-2">
            <div>
              <img src="https://res.cloudinary.com/sayed-ltd/image/upload/w_1000,ar_16:9,c_fill,g_auto,e_sharpen/v1724601933/logan-weaver-lgnwvr-0fiuoIIfl9g-unsplash_1_moiaxv.jpg" alt="" />
            </div>
            <div className="flex">
                <div>
                    <h1>KTM Duke 1290</h1>
                    <p>location</p>
                    <button className="btn bg-red-500 text-white py-2 px-4 mt-3 font-bold">Book Now</button>
                </div>
                <div className="ml-auto text-right">
                    <h3>$100</h3>
                    <p className="text-sm">DAY</p>
                    <p className="mt-8">991 Reviews <FontAwesomeIcon className="text-yellow-500" icon={faStar} /> 5.0</p>
                </div>
            </div>
          </div>
          <div className="grid-cols-1 border border-red-500 p-5 border-r-2">
            <div>
              <img src="https://res.cloudinary.com/sayed-ltd/image/upload/w_1000,ar_16:9,c_fill,g_auto,e_sharpen/v1724601933/logan-weaver-lgnwvr-0fiuoIIfl9g-unsplash_1_moiaxv.jpg" alt="" />
            </div>
            <div className="flex">
                <div>
                    <h1>KTM Duke 1290</h1>
                    <p>location</p>
                    <button className="btn bg-red-500 text-white py-2 px-4 mt-3 font-bold">Book Now</button>
                </div>
                <div className="ml-auto text-right">
                    <h3>$100</h3>
                    <p className="text-sm">DAY</p>
                    
                    <p className="mt-8">991 Reviews <FontAwesomeIcon className="text-yellow-500" icon={faStar} /> 5.0</p>
                </div>
            </div>
          </div>
          <div className="grid-cols-1 border border-red-500 p-5 border-r-2">
            <div>
              <img src="https://res.cloudinary.com/sayed-ltd/image/upload/w_1000,ar_16:9,c_fill,g_auto,e_sharpen/v1724601933/logan-weaver-lgnwvr-0fiuoIIfl9g-unsplash_1_moiaxv.jpg" alt="" />
            </div>
            <div className="flex">
                <div>
                    <h1>KTM Duke 1290</h1>
                    <p>location</p>
                    <button className="btn bg-red-500 text-white py-2 px-4 mt-3 font-bold">Book Now</button>
                </div>
                <div className="ml-auto text-right">
                    <h3>$100</h3>
                    <p className="text-sm">DAY</p>
                    <p className="mt-8">991 Reviews <FontAwesomeIcon className="text-yellow-500" icon={faStar} /> 5.0</p>
                </div>
            </div>
          </div>
          <div className="grid-cols-1 border border-red-500 p-5 border-r-2">
            <div>
              <img src="https://res.cloudinary.com/sayed-ltd/image/upload/w_1000,ar_16:9,c_fill,g_auto,e_sharpen/v1724601933/logan-weaver-lgnwvr-0fiuoIIfl9g-unsplash_1_moiaxv.jpg" alt="" />
            </div>
            <div className="flex">
                <div>
                    <h1>KTM Duke 1290</h1>
                    <p>location</p>
                    <button className="btn bg-red-500 text-white py-2 px-4 mt-3 font-bold">Book Now</button>
                </div>
                <div className="ml-auto text-right">
                    <h3>$100</h3>
                    <p className="text-sm">DAY</p>
                    <p className="mt-8">991 Reviews <FontAwesomeIcon className="text-yellow-500" icon={faStar} /> 5.0</p>
                </div>
            </div>
          </div>
          <div className="grid-cols-1 border border-red-500 p-5 border-r-2 ">
            <div>
              <img src="https://res.cloudinary.com/sayed-ltd/image/upload/w_1000,ar_16:9,c_fill,g_auto,e_sharpen/v1724601933/logan-weaver-lgnwvr-0fiuoIIfl9g-unsplash_1_moiaxv.jpg" alt="" />
            </div>
            <div className="flex">
                <div>
                    <h1>KTM Duke 1290</h1>
                    <p>location</p>
                    <button className="btn bg-red-500 text-white py-2 px-4 mt-3 font-bold">Book Now</button>
                </div>
                <div className="ml-auto text-right">
                    <h3>$100</h3>
                    <p className="text-sm">DAY</p>
                    <p className="mt-8">991 Reviews <FontAwesomeIcon className="text-yellow-500" icon={faStar} /> 5.0</p>
                </div>
            </div>
          </div>

          
        </div>
        <div className="col-span-1"></div>
      </div>
    </div>
  );
};

export default RentCards;
